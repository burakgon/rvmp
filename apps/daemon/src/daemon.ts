 ...
 * installed here — there is exactly one daemon per process. */
export async function startDaemon(): Promise<{ url: string; token: string; stop: () => Promise<void> }> {
  ...
  return { url: srv.url, token: cfg.token, stop: shutdown };
}
